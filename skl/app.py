...
def solve_system_crt(congruences):
    ...
    for i in range(len(moduli)):
        for j in range(i + 1, len(moduli)):
            if math.gcd(moduli[i], moduli[j]) != 1:
                ...
                steps.append(f"❌ **Gagal Cek CRT**: Moduli ${moduli[i]}$ dan ${moduli[j]}$ tidak relatif prima karena $\\text{GCD}({moduli[i]}, {moduli[j]}) = {math.gcd(moduli[i], moduli[j])}$. Solusi mungkin tidak ada atau Teorema Sisa China tidak dapat diterapkan secara langsung.")
                return None, False, steps # Mengembalikan None jika CRT tidak berlaku
    
    if is_crt_applicable and len(congruences) > 1:
        steps.append("✅ **Cek CRT Sukses**: Semua moduli relatif prima. Teorema Sisa China dapat diterapkan.")
    ...
    for i in range(1, len(congruences)):
        ...
        if status == "TIDAK ADA SOLUSI":
            steps.append(f"❌ **Gagal**: Kongruensi $k \\cdot {A} \\equiv {B} \\pmod{{{M}}}$ tidak memiliki solusi (karena $\\text{GCD}({A}, {M}) = {g}$ dan ${B}$ tidak habis dibagi ${g}$).")
            return None, False, steps
        
        # Solusi untuk k: k = k0_prime (mod m_prime)
        steps.append(f"✅ **Solusi k**: $k \\equiv {k0_prime} \\pmod{{{m_prime}}}$.")
...
st.set_page_config(
    page_title="🔬 Virtual Lab Sistem Kongruensi Linier (CRT)",
    layout="wide",
    initial_sidebar_state="expanded"
)

st.title("🔬 Virtual Lab: Sistem Kongruensi Linier")
...
with col_btn1:
    if st.button("➕ Tambah Kongruensi", key="add_btn"):
        st.session_state.num_congruences += 1
with col_btn2:
    if st.session_state.num_congruences > 1 and st.button("➖ Kurangi Kongruensi", key="rem_btn"):
        st.session_state.num_congruences -= 1
...
if st.button("Hitung Solusi Sistem", type="primary"):
    ...
    st.subheader("📝 Sistem Kongruensi yang Diberikan")
    ...
    st.subheader("💡 Hasil dan Langkah Penyelesaian")
    ...
    if final_x is not None:
        ...
        st.success(f"### 🎉 SOLUSI AKHIR DITEMUKAN!")
        st.markdown(
            f"""
        ...
        st.error("### 🚫 TIDAK ADA SOLUSI atau CRT Tidak Dapat Diterapkan Secara Langsung")
